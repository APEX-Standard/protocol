...
pub fn handle_order_place(
    ...
    if order["instrument_id"].as_str() != Some(INSTRUMENT_ID) {
...
const FUTURES_ROOT_ID: &str = "APEX:FUT:ES";
const FUTURES_EXPIRED_ID: &str = "APEX:FUT:ESU26";
const FUTURES_FRONT_MONTH_ID: &str = "APEX:FUT:ESZ26";
const FUTURES_NEXT_MONTH_ID: &str = "APEX:FUT:ESH27";

pub fn handle_futures_contract_chain(root: &str, include_expired: bool) -> Value {
    if root.is_empty() {
        return serde_json::to_value(apex_error(
            "APEX_4011",
            "validation",
...
pub fn handle_futures_margin_schedule(account_id: &str, instrument_id: &str) -> Value {
    if account_id.is_empty() {
        return serde_json::to_value(apex_error(
            "APEX_4011",
            "validation",
    ...
    if !instrument_id.is_empty() && instrument_id != FUTURES_FRONT_MONTH_ID {
        return serde_json::to_value(apex_error(
            "APEX_4010",
            "validation",
            "Unknown instrument",
